...
// Configuration
const WIDGET_SERVER_PORT = process.env.WIDGET_PORT || 3001;
...
// Create Express server for widget assets
const app = express();
app.use(cors());
...
app.get('/', (req, res) => {
  res.json({
    name: 'Tutorial Video Apps SDK Server',
    version: '1.0.0',
    widgetUrl: `${WIDGET_BASE_URL}/widgets`,
  });
});

// Start widget server
const widgetServer = app.listen(WIDGET_SERVER_PORT, () => {
  console.error(`✓ Widget server running on port ${WIDGET_SERVER_PORT}`);
  console.error(`  Widget URL: ${WIDGET_BASE_URL}/widgets`);
});
...
async function startMCPServer() {
  console.error('Starting Tutorial Video Apps SDK MCP Server...');
  ...
  console.error('\n📘 Apps SDK Mode:');
  console.error('  - Videos will display inline using widgets');
  console.error('  - Widget URL: ' + WIDGET_BASE_URL + '/widgets/video-player.html');
  console.error('\n💡 To test in ChatGPT:');
  console.error('  1. Enable Developer Mode in ChatGPT');
  console.error('  2. Add this MCP server');
